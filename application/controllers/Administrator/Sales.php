...
class Sales extends CI_Controller {
        ...
        try{
            ...
            foreach($data->cart as $cartProduct){
                $saleDetails = array(
                    ...
                    'discount_qty' => $cartProduct->discount_qty,
                ...
                $this->db->query("
                    ...
                    where product_id = ?
                    and branch_id = ?
                ", [($cartProduct->quantity+$cartProduct->discount_qty), $cartProduct->productId, $this->session->userdata('BRANCHid')]);
            }
        ...
        try{
            ...
            foreach($currentSaleDetails as $product){
                $this->db->query("
                    ...
                    where product_id = ?
                    and branch_id = ?
                ", [($product->SaleDetails_TotalQuantity + $product->discount_qty), $product->Product_IDNo, $this->session->userdata('BRANCHid')]);
            }
    ...
    public function  deleteSales(){
        ...
        try{
            ...
            $saleDetails = $this->db->select('Product_IDNo, SaleDetails_TotalQuantity,discount_qty')->where('SaleMaster_IDNo', $saleId)->get('tbl_saledetails')->result();
            ...
            foreach ($saleDetails as $detail){
                ...
                $newQty = $totalQty - ($detail->SaleDetails_TotalQuantity+$detail->discount_qty);
