...
class Purchase extends CI_Controller
{
            ...
            foreach($data->cartProducts as $product){
                ...
                if($inventoryCount == 0){
                    $inventory = array(
                        ...
                        'purchase_quantity' => ($product->quantity + $product->discount_qty),
                    ...
                    $this->db->query("
                        ...
                        where product_id = ? 
                        and branch_id = ?
                    ", [($product->quantity+$product->discount_qty), $product->productId, $this->session->userdata('BRANCHid')]);
                }
        ...
        try{
            ...
            foreach($oldPurchaseDetails as $product){
                $this->db->query("
                    ...
                    where product_id = ?
                    and branch_id = ?
                ", [($product->PurchaseDetails_TotalQuantity + $product->discount_qty), $product->Product_IDNo, $this->session->userdata('BRANCHid')]);
            }
    ...
    public function  deletePurchase(){
        ...
        try{
            ...
            $purchaseDetails = $this->db->select('Product_IDNo,PurchaseDetails_TotalQuantity,discount_qty')->where('PurchaseMaster_IDNo',$data->purchaseId)->get('tbl_purchasedetails')->result();
            ...
            foreach ($purchaseDetails as $detail){
                ...
                $newQty = $totalQty - ($detail->PurchaseDetails_TotalQuantity+$detail->discount_qty);
