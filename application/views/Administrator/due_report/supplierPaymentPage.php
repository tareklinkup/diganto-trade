			...
			<form @submit.prevent="saveSupplierPayment">
				<div class="row">
					<div class="col-md-5 col-md-offset-1">
						...
						<div class="form-group">
							<label class="col-md-4 control-label">{{ payment.supplierDue > 0 ? 'Due' : 'Advance' }} </label>
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<input type="text" v-if="payment.supplierDue > 0" readonly class="form-control" v-model="payment.supplierDue">
								<input type="text" v-else readonly class="form-control" v-model="payment.supplierDue * -1">
							</div>
						</div>
					</div>
		...
		methods:{
			...
			getSupplierDue(){
				...
				axios.post('/get_supplier_due', {supplierId: this.selectedSupplier.Supplier_SlNo}).then(res => {
					this.payment.supplierDue = res.data[0].due;
				})
			},
			getAccounts(){
