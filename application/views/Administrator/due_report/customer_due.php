			...
			<table class="table table-bordered">
				...
				<tbody>
					<tr v-for="data in dues">
						...
						<td v-if="data.dueAmount < 0" style="text-align:right">{{ parseFloat(data.dueAmount).toFixed(2) *-1 }}</td>
						<td v-else style="text-align:right">0.00</td>
						<td v-if="data.dueAmount > 0" style="text-align:right">{{ parseFloat(data.dueAmount).toFixed(2) }}</td>
						<td v-else style="text-align:right">0.00</td>
					</tr>
				</tbody>
				<tfoot>
		...
		methods:{
			...
			getDues(){
				...
				axios.post('/get_customer_due', {customerId: customerId, districtId: districtId}).then(res => {
					...
					const dueAmount = this.dues.filter(payment => {
					...
					const advancePayment = this.dues.filter(payment => {
						if(payment.dueAmount < 0){
							return payment
						}
					});
					
					this.totalAdvance = advancePayment.reduce((prev, curr) =>  prev+ (parseFloat(curr.dueAmount) *-1),0);
					this.totalDue = dueAmount.reduce((prev, curr) =>  prev+ (parseFloat(curr.dueAmount)),0);
				})
			},
			async print(){
				let reportContent = `
