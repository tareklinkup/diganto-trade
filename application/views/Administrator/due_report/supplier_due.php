				...
				<table class="table table-bordered">
					...
					<tbody>
						<tr v-for="due in dueList">
							...
							<td v-if="due.due < 0">{{ parseFloat(due.due).toFixed(2) *-1 }}</td>
							<td v-else>0.00</td>

							<td v-if="due.due > 0">{{ parseFloat(due.due).toFixed(2) }}</td>
							<td v-else>0.00</td>
						</tr>
					</tbody>
					<tbody>
		...
		methods: {
			...
			getDues() {
				...
				axios.post('/get_supplier_due', {
					...
					const advancePayment = this.dueList.filter(payment => {
						if(payment.due < 0){
							return payment
						}
					});
					const totalDue = this.dueList.filter(payment => {
						if(payment.due > 0){
							return payment
						}
					});

					this.totalAdvance = advancePayment.reduce((prev, curr) =>  prev+ (parseFloat(curr.due) *-1),0);
					this.dueAmount = totalDue.reduce((prev, curr) =>  prev+ (parseFloat(curr.due)),0);
				})
			},
			async print(){
				let reportContent = `
