						...
						<div class="col-sm-5">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group">
									...
									<div class="col-sm-3">
										<input type="number" step="0.001" id="quantity" placeholder="Qty" class="form-control" ref="quantity" v-model="selectedProduct.quantity" v-on:input="productTotal" autocomplete="off" required/>
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-2 control-label no-padding-right"> Shinkrag </label>
									<div class="col-sm-4">
										<input type="number" step="0.001" id="discount_qty" placeholder="Qty" class="form-control" ref="discount_qty" v-model="selectedProduct.discount_qty" v-on:input="productTotal" autocomplete="off"/>
									</div>
								</div>
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					<thead>
						<tr class="">
							...
							<th style="width:7%;color:#000;">Shinkrag</th>
							...
							<div class="table-responsive">
								<table style="color:#000;margin-bottom: 0px;border-collapse: collapse;">
									<tr>
										<td>
											<div class="form-group">
												<label class="col-sm-12 control-label no-padding-right">Advance Amount</label>
												<div class="col-sm-12">
													<input type="number" id="subTotal" class="form-control" v-model="sales.advanceAmount" readonly />
												</div>
											</div>
										</td>
									</tr>
		...
		methods:{
			...
			async customerOnChange(){
				...
				await axios.post('/get_customer_due',{customerId: this.selectedCustomer.Customer_SlNo}).then(res=>{
					if(res.data.length > 0){
						if(res.data[0].dueAmount > 0)
							this.sales.previousDue = res.data[0].dueAmount;
						else{
							this.sales.previousDue = res.data[0].dueAmount;
							this.sales.advanceAmount = parseFloat(res.data[0].dueAmount) * -1;
						}
			...
			addToCart(){
				let product = {
					...
					discount_qty: this.selectedProduct.discount_qty == undefined ? 0 : this.selectedProduct.discount_qty
				}
			...
			calculateTotal(){
				...
				this.sales.total = ((parseFloat(this.sales.subTotal) + parseFloat(this.sales.vat)) - parseFloat(this.sales.discount)).toFixed(2);
				...
				// if(this.sales.total <= this.sales.advanceAmount) {
				// 	this.sales.paid = this.sales.total;
				// }else{
				// 	this.sales.paid = 0
				// }
				...
				axios.post('/get_sales', {salesId: this.sales.salesId}).then(res=>{
					...
					r.saleDetails.forEach(product => {
						let cartProduct = {
							...
							discount_qty: product.discount_qty
						}
